...
HARDEN_WIN_LOG_CMD = $(POWERSHELL) -NoProfile -ExecutionPolicy Bypass -File scripts/harden-win-log.ps1
...
VERIFY_GENERATED_LOG_CMD = $(POWERSHELL) -NoProfile -ExecutionPolicy Bypass -File scripts/verify-generated-log.ps1
...
ifeq ($(PLATFORM),windows)
$(SRC): $(SRC_INPUT) scripts/gen-win-port.ps1 scripts/harden-win-path.ps1 scripts/harden-win-io.ps1 scripts/harden-win-request.ps1 scripts/harden-win-log.ps1 | $(BUILD_DIR)
	$(GEN_WIN_CMD)
	$(HARDEN_WIN_PATH_CMD)
	$(HARDEN_WIN_IO_CMD)
