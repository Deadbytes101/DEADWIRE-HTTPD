...
That executable opens loopback through a V2 long-mode controller, runs a bounded smoke loop, sends four GET requests one by one, runs bounded V2 mode for each request, checks each HTTP 200 response and body, proves the queue cursor wraps back to zero, closes sockets, and exits nonzero on failure.
