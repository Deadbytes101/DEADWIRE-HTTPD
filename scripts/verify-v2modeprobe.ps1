...
foreach($P in @($RT,$LV,$AC,$BR,$TK,$BD,$MO,$CL)){if(!(Test-Path $P)){throw "missing $P"}}
$RO=Join-Path $B 'v2mp_runtime.o'; $LO=Join-Path $B 'v2mp_live.o'; $AO=Join-Path $B 'v2mp_accept.o'; $BO=Join-Path $B 'v2mp_bridge.o'; $TO=Join-Path $B 'v2mp_tick.o'; $BDO=Join-Path $B 'v2mp_bound.o'; $MOO=Join-Path $B 'v2mp_mode.o'; $CLO=Join-Path $B 'v2mp_close.o'
...
int main(void){
 ...
 uint64_t w[5]={0},live[5]={0,0,0,0,99},client[4]={99,0,0,0},tc[7]={0},bc[4]={0},mc[2]={0};
 ...
 if(dw_runtime_mode_bound(mc))return 6;
 if(mc[1]!=0||bc[2]!=1||bc[3]!=0)return 7;
