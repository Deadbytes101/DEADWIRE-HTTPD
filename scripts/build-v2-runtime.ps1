...
foreach ($Path in @($RuntimePath, $LaneSetPath, $ExtraPath)) {
...
& gcc -nostdlib '-Wl,-e,mainCRTStartup' '-Wl,--subsystem,console' -o $ExePath $BootObjectPath $RuntimeObjectPath $LaneSetObjectPath $ExtraObjectPath -lws2_32 -lkernel32
if ($LASTEXITCODE -ne 0) {
