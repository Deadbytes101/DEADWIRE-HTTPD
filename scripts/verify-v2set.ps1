...
$RuntimeObjectPath = Join-Path $BuildDir 'runtime_v2set_base.o'
$SetObjectPath = Join-Path $BuildDir 'runtime_v2set.o'
$HarnessPath = Join-Path $BuildDir 'verify_runtime_v2set.s'
$HarnessObjectPath = Join-Path $BuildDir 'verify_runtime_v2set.o'
$HarnessExePath = Join-Path $BuildDir 'verify_runtime_v2set.exe'
...
foreach ($Needle in $RequiredNeedles) {
    if (-not $Source.Contains($Needle)) {
        throw "missing V2 lane start logic: $Needle"
    }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "V2 lane start assembly failed with exit code $LASTEXITCODE"
}

$SymbolLines = & nm -g $SetObjectPath
if ($LASTEXITCODE -ne 0) {
    throw "V2 lane start symbol table failed with exit code $LASTEXITCODE"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "V2 lane start harness assembly failed with exit code $LASTEXITCODE"
}

& gcc -nostdlib '-Wl,-e,mainCRTStartup' -o $HarnessExePath $HarnessObjectPath $SetObjectPath $RuntimeObjectPath -lws2_32 -lkernel32
if ($LASTEXITCODE -ne 0) {
    throw "V2 lane start harness link failed with exit code $LASTEXITCODE"
}

& $HarnessExePath
if ($LASTEXITCODE -ne 0) {
    throw "V2 lane start harness failed with exit code $LASTEXITCODE"
}

Write-Output 'verify-v2set: ok'
