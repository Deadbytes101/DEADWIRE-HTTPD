...
Has '$CompileArgs = @(' 'portable compile args'
Has 'RuntimeInformation]::IsOSPlatform' 'host platform check'
Has "if (`$IsWinHost) { `$CompileArgs += '-lws2_32' }" 'windows socket library guard'
Has "if (!`$IsWinHost) { `$CompileArgs = @('-D_POSIX_C_SOURCE=200809L') + `$CompileArgs }" 'linux posix compile definition'
Has '& $cc @CompileArgs' 'portable client compile'
