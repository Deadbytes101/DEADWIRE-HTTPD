...
Has $RequestS 'run_case(&b,0,health_req,(uint64_t)health_resp' 'request health route'
Has $RequestS 'run_case(&b,1,root_req,(uint64_t)root_resp' 'request root route'
Has $RequestS 'run_case(&b,2,css_req,(uint64_t)css_resp' 'request css route'
Has $RequestS 'run_case(&b,3,missing_req,(uint64_t)missing_resp' 'request missing route'
Has $RequestS 'client[8]={99,(uint64_t)reqbuf,sizeof(reqbuf),0,(uint64_t)health_resp,(uint64_t)root_resp,(uint64_t)css_resp,(uint64_t)missing_resp}' 'request response table client'
Has $RequestS 'if(client[3])return 60+index;' 'request starts without selected response'
Has $RequestS 'if(dw_runtime_http_request_step(worker))return 80+index;' 'request http engine call'
Has $RequestS 'if(client[3]!=expected)return 90+index;' 'request selected response result'
