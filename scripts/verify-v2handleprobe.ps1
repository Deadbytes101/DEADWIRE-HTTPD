...
foreach($P in @($Gen,$ROUTE,$LV,$AC,$CL)){if(!(Test-Path $P)){throw "missing $P"}}
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $Gen
if($LASTEXITCODE){throw "gen hot $LASTEXITCODE"}
$RO=Join-Path $B 'v2hp_hot.o'; $RTO=Join-Path $B 'v2hp_route.o'; $LO=Join-Path $B 'v2hp_live.o'; $AO=Join-Path $B 'v2hp_accept.o'; $CO=Join-Path $B 'v2hp_close.o'
& as --64 -o $RO $Hot; if($LASTEXITCODE){throw 'as hot'}
...
int main(void){
 ...
 if(dw_runtime_handle_client(client))return 8;
 if(client[3]!=(uint64_t)resp)return 9;
