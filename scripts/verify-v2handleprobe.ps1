...
foreach($P in @($RT,$LV,$AC,$CL)){if(!(Test-Path $P)){throw "missing $P"}}
$RO=Join-Path $B 'v2hp_runtime.o'; $LO=Join-Path $B 'v2hp_live.o'; $AO=Join-Path $B 'v2hp_accept.o'; $CO=Join-Path $B 'v2hp_close.o'
...
int main(void){
 ...
 client[1]=(uint64_t)reqbuf; client[2]=sizeof(reqbuf); client[3]=(uint64_t)resp;
 if(send(out,req,(int)(sizeof(req)-1),0)!=(int)(sizeof(req)-1))return 6;
 if(dw_runtime_handle_client(client))return 7;
 int n=recv(out,outbuf,sizeof(outbuf)-1,0); if(n<=0)return 8; outbuf[n]=0;
 if(!strstr(outbuf,"HTTP/1.1 200 OK"))return 9;
 if(!strstr(outbuf,"V2 OK"))return 10;
...
& gcc -o $EX $OBJ $RO $LO $AO $CO -lws2_32 -lkernel32; if($LASTEXITCODE){throw 'link'}
