...
foreach($P in @($RT,$LV,$AC,$BR,$TK,$CL)){if(!(Test-Path $P)){throw "missing $P"}}
$RO=Join-Path $B 'v2cp_runtime.o'; $LO=Join-Path $B 'v2cp_live.o'; $AO=Join-Path $B 'v2cp_accept.o'; $BO=Join-Path $B 'v2cp_bridge.o'; $TO=Join-Path $B 'v2cp_tick.o'; $CLO=Join-Path $B 'v2cp_close.o'
...
& as --64 -o $TO $TK; if($LASTEXITCODE){throw 'as cycle'}
& as --64 -o $CLO $CL; if($LASTEXITCODE){throw 'as close'}
$C=Join-Path $B 'verify_runtime_v2cycleprobe.c'
$CO=Join-Path $B 'verify_runtime_v2cycleprobe.o'
$EX=Join-Path $B 'verify_runtime_v2cycleprobe.exe'
@'
#include <stdint.h>
#include <winsock2.h>
#include <ws2tcpip.h>
...
int main(void){
 ...
 if(dw_runtime_tick_once(tc))return 6;
 if(tc[5]!=(uint64_t)client||tc[6]!=0)return 7;
 if(w[4]!=1||iq[0]!=1||iq[1]!=1||oq[0]!=1||oq[1]!=1)return 8;
 closesocket((SOCKET)client[0]); closesocket(out);
 if(dw_runtime_live_close(live))return 9;
